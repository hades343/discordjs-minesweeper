...
const COMMANDS_CACHE = fs.existsSync(COMMANDS_CACHE_FILE)
	? JSON.parse(fs.readFileSync(COMMANDS_CACHE_FILE).toString())
	: null;
...
async function registerSlashCommands(clientId, token) {
	...
	if (!COMMANDS_CACHE || JSON.stringify(commands) !== JSON.stringify(COMMANDS_CACHE)) {
